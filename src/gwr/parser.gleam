...
pub fn parse_locals_declaration(from reader: byte_reader.ByteReader) -> Result(#(byte_reader.ByteReader, spec.LocalsDeclaration), parsing_error.ParsingError)
{
...
pub fn parse_function_code(from reader: byte_reader.ByteReader) -> Result(#(byte_reader.ByteReader, spec.FunctionCode), parsing_error.ParsingError)
{
    use #(reader, locals_declaration) <- result.try(parse_vector(from: reader, with: parse_locals_declaration))
...
pub fn parse_code(from reader: byte_reader.ByteReader) -> Result(#(byte_reader.ByteReader, spec.Code), parsing_error.ParsingError)
{
...
pub fn parse_section(from reader: byte_reader.ByteReader) -> Result(#(byte_reader.ByteReader, spec.Section), parsing_error.ParsingError)
{
    use #(reader, section_type_id)  <- result.try(
        case byte_reader.read(from: reader, take: 1)
        {
            ...
            _ -> parsing_error.new()
                 |> parsing_error.add_message("gwr/parser/parser.parse_section: can't get section type id raw data")
                 |> parsing_error.to_error()
        }
    )

    use #(reader, section_length) <- result.try(parse_unsigned_leb128_integer(from: reader))
    ...
    use <- bool.guard(
        ...
        return: parsing_error.new()
                |> parsing_error.add_message("gwr/parser/parser.parse_section: unexpected end of the section's content segment. Expected " <> int.to_string(section_length) <> " bytes but got " <> int.to_string(remaining_data_length) <> " bytes")
                |> parsing_error.to_error()
    )
    ...
    use #(reader, decoded_dection) <- result.try(
        case byte_reader.can_read(reader), section_type_id // the reverse order throws a syntax error xD
        {
            ...
            {
                use #(reader, indices_vec) <- result.try(parse_vector(from: reader, with: fn (reader) {
                    use #(reader, index) <- result.try(parse_unsigned_leb128_integer(from: reader))
                    Ok(#(reader, index))
                }))
...
pub fn parse_binary_module(from reader: byte_reader.ByteReader) -> Result(#(byte_reader.ByteReader, spec.Binary), parsing_error.ParsingError)
{
    use <- bool.guard(when: byte_reader.is_empty(reader), return:
        parsing_error.new()
        |> parsing_error.add_message("gwr/parser/parser.parse_binary_module: empty data")
        |> parsing_error.to_error()
    )
    ...
    use <- bool.guard(when: !found_magic_number, return:
        parsing_error.new()
        |> parsing_error.add_message("gwr/parser/parser.parse_binary_module: couldn't find module's magic number")
        |> parsing_error.to_error()
    )
    ...
    use #(reader, module_wasm_version) <- result.try(
        case byte_reader.read(from: reader, take: 4)
        {
            ...
            _ -> parsing_error.new()
                 |> parsing_error.add_message("gwr/parser/parser.parse_binary_module: couldn't find module version")
                 |> parsing_error.to_error()
        }
    )

    let empty_module = spec.Module
    (
        types: [],
        functions: [],
        tables: [],
                ...
                use #(module, function_section_type_indices) <- result.try(
                    case section.content
                    {
                        ...
                        Some(content) ->
                        {
                            case content
                            {
                                spec.CustomSection(name: _, data: _) -> Ok(#(module, function_section_type_indices))
                                spec.TypeSection(function_types: function_types) ->
                                {
                                    Ok(#(spec.Module(..module, types: function_types), function_section_type_indices))
                                }
                                spec.ImportSection -> Ok(#(module, function_section_type_indices))
                                spec.FunctionSection(type_indices: type_indices_list) ->
                                {
                                    // Here we set function_section_type_indices
                                    Ok(#(module, type_indices_list))
                                }
                                spec.TableSection -> Ok(#(module, function_section_type_indices))
                                spec.MemorySection(memories: memories_list) ->
                                {
                                    Ok(#(spec.Module(..module, memories: memories_list), function_section_type_indices))
                                }
                                spec.GlobalSection(globals: globals_list) ->
                                {
                                    Ok(#(spec.Module(..module, globals: globals_list), function_section_type_indices))
                                }
                                spec.ExportSection(exports: exports_list) ->
                                {
                                    Ok(#(spec.Module(..module, exports: exports_list), function_section_type_indices))
                                }
                                            ...
                                            with: fn(function_list, entry, index)
                                            {
                                                ...
                                                case function_section_type_indices |> list.take(up_to: index + 1) |> list.last
                                                {
                                                    ...
                                                    Error(_) -> parsing_error.new()
                                                                |> parsing_error.add_message("gwr/parser/parser.parse_binary_module: couldn't find type index " <> int.to_string(index))
                                                                |> parsing_error.to_error()
                                                }
                                            }
                                        )
                                    )
                                    Ok(#(spec.Module(..module, functions: function_list), function_section_type_indices))
                                }
                                spec.DataSection -> Ok(#(module, function_section_type_indices))
                                spec.DataCountSection -> Ok(#(module, function_section_type_indices))
                            }
                        }
                    }
                )
    ...
    Ok(#(reader, spec.Binary(version: module_wasm_version, length: byte_reader.bytes_read(from: reader), module: filled_module)))
}
...
) -> Result(#(byte_reader.ByteReader, spec.Vector(a)), parsing_error.ParsingError)
{
    use #(reader, vector_length) <- result.try(parse_unsigned_leb128_integer(from: reader))
...
pub fn parse_instruction(from reader: byte_reader.ByteReader) -> Result(#(byte_reader.ByteReader, spec.Instruction), parsing_error.ParsingError)
{
    use #(reader, opcode) <- result.try(
        case byte_reader.read(from: reader, take: 1)
        {
            ...
            _ -> parsing_error.new()
                 |> parsing_error.add_message("gwr/parser/parse_instruction: unknown error reading opcode")
                 |> parsing_error.to_error()
        }
    )
    ...
    use #(reader, instruction) <- result.try(
        case opcode
        {
            ...
            0x04 ->
            {
                ...
                case list.last(body)
                {
                    Ok(spec.End) -> Ok(#(reader, spec.If(block_type: block_type, instructions: body, else_: option.None)))
                    Ok(spec.Else(_) as els) -> Ok(#(reader, spec.If(block_type: block_type, instructions: list.take(from: body, up_to: list.length(body) - 1), else_: option.Some(els))))
                    _ -> parsing_error.new()
                         |> parsing_error.add_message("gwr/parser/parse_instruction: expected the If instruction's block to end either with an End instruction or an Else instruction")
                         |> parsing_error.to_error()
                }
            }
            0x05 ->
            ...
            0xc ->
            {
                use #(reader, label_index) <- result.try(parse_unsigned_leb128_integer(from: reader))
            ...
            0x10 ->
            {
                use #(reader, function_index) <- result.try(parse_unsigned_leb128_integer(from: reader))
            ...
            0x20 ->
            {
                use #(reader, local_index) <- result.try(parse_unsigned_leb128_integer(from: reader))
            ...
            0x41 ->
            {
                use #(reader, value) <- result.try(parse_uninterpreted_leb128_integer(from: reader))
            ...
            0xfc ->
            {
                use #(reader, actual_opcode) <- result.try(parse_unsigned_leb128_integer(from: reader))
                case actual_opcode
                {
                    ...
                    unknown -> parsing_error.new()
                               |> parsing_error.add_message("gwr/parser/parse_instruction: unknown saturating truncation instruction opcode \"0x" <> int.to_base16(unknown) <> "\"")
                               |> parsing_error.to_error()
                }
            }
            // End
            ...
            unknown -> parsing_error.new()
                       |> parsing_error.add_message("gwr/parser/parse_instruction: unknown opcode \"0x" <> int.to_base16(unknown) <> "\"")
                       |> parsing_error.to_error()
        }
    )
...
pub fn parse_expression(from reader: byte_reader.ByteReader) -> Result(#(byte_reader.ByteReader, spec.Expression), parsing_error.ParsingError)
{
    ...
    use #(reader, expression) <- result.try(
        yielder.fold(
            ...
            with: fn (state, _) {
                ...
                use <- bool.guard(when: list.last(current_expression) == Ok(spec.End), return: state)
                
                // If we reached the end of the data then the last instruction there must be an End instruction; otherwise we got an error 
                use <- bool.guard(when: !byte_reader.can_read(reader) && list.last(current_expression) != Ok(spec.End), return:
                    parsing_error.new()
                    |> parsing_error.add_message("gwr/parser/parse_expression: an expression must terminate with a End instruction")
                    |> parsing_error.to_error()
                )
...
pub fn do_parse_instructions_until(reader: byte_reader.ByteReader, predicate: fn (spec.Instruction) -> Bool, accumulator: List(spec.Instruction)) -> Result(#(byte_reader.ByteReader, List(spec.Instruction)), parsing_error.ParsingError)
{
    case byte_reader.can_read(reader)
    {
        ...
        False -> parsing_error.new()
                 |> parsing_error.add_message("gwr/parser/do_parse_instructions_until: reached the end of the data yet couldn't find the instruction matching the given predicate")
                 |> parsing_error.to_error()
    }
}

pub fn parse_instructions_until(from reader: byte_reader.ByteReader, with predicate: fn (spec.Instruction) -> Bool) -> Result(#(byte_reader.ByteReader, List(spec.Instruction)), parsing_error.ParsingError)
{
    do_parse_instructions_until(reader, predicate, [])
}

pub fn parse_block_type(from reader: byte_reader.ByteReader) -> Result(#(byte_reader.ByteReader, spec.BlockType), parsing_error.ParsingError)
{
    ...
    case is_value_type(first_byte)
    {
        ...
        False ->
        {
            case first_byte
            {
                <<0x40>> -> Ok(#(byte_reader.advance(from: reader, up_to: 1), spec.ValueTypeBlock(type_: option.None)))
                _ ->
                {
                    use #(reader, index) <- result.try(parse_signed_leb128_integer(from: reader))
...
pub fn parse_export(from reader: byte_reader.ByteReader) -> Result(#(byte_reader.ByteReader, spec.Export), parsing_error.ParsingError)
{
    ...
    use #(reader, export) <- result.try(
        case byte_reader.read(from: reader, take: 1)
        {
            ...
            {
                use #(reader, index) <- result.try(parse_unsigned_leb128_integer(from: reader))
                Ok(#(reader, spec.Export(name: export_name, descriptor: spec.FunctionExport(index))))
            }
            Ok(#(reader, <<id>>)) if id == table_index_id ->
            {
                use #(reader, index) <- result.try(parse_unsigned_leb128_integer(from: reader))
                Ok(#(reader, spec.Export(name: export_name, descriptor: spec.TableExport(index))))
            }
            Ok(#(reader, <<id>>)) if id == memory_index_id ->
            {
                use #(reader, index) <- result.try(parse_unsigned_leb128_integer(from: reader))
                Ok(#(reader, spec.Export(name: export_name, descriptor: spec.MemoryExport(index))))
            }
            Ok(#(reader, <<id>>)) if id == global_index_id ->
            {
                use #(reader, index) <- result.try(parse_unsigned_leb128_integer(from: reader))
                Ok(#(reader, spec.Export(name: export_name, descriptor: spec.GlobalExport(index))))
            }
            Ok(#(_, <<unknown>>)) -> parsing_error.new()
                                     |> parsing_error.add_message("gwr/parser/parse_export: unexpected export id \"" <> int.to_string(unknown) <> "\"")
                                     |> parsing_error.to_error()
            Error(reason) -> Error(reason)
            _ -> parsing_error.new()
                 |> parsing_error.add_message("gwr/parser/parse_export: unknown error reading export id")
                 |> parsing_error.to_error()
        }
    )
...
pub fn parse_global(from reader: byte_reader.ByteReader) -> Result(#(byte_reader.ByteReader, spec.Global), parsing_error.ParsingError)
{
...
pub fn parse_memory(from reader: byte_reader.ByteReader) -> Result(#(byte_reader.ByteReader, spec.Memory), parsing_error.ParsingError)
{
...
pub fn parse_value_type(from reader: byte_reader.ByteReader) -> Result(#(byte_reader.ByteReader, spec.ValueType), parsing_error.ParsingError)
{
    use #(reader, value_type_id) <- result.try(
        case byte_reader.read(from: reader, take: 1)
        {
            ...
            Error(reason) -> Error(reason)
            _ -> parsing_error.new()
                 |> parsing_error.add_message("gwr/parser/parse_value_type: unknown error reading value type id")
                 |> parsing_error.to_error()
        }
    )
    ...
    use value_type <- result.try(
        case value_type_id
        {
            ...
            unknown -> parsing_error.new()
                       |> parsing_error.add_message("gwr/parser/parse_value_type: unknown value type \"" <> int.to_string(unknown) <> "\"")
                       |> parsing_error.to_error()
        }
    )
...
pub fn parse_limits(from reader: byte_reader.ByteReader) -> Result(#(byte_reader.ByteReader, spec.Limits), parsing_error.ParsingError)
{
    ...
    use #(reader, has_max) <- result.try(
        case byte_reader.read(from: reader, take: 1)
        {
            ...
            Ok(#(_, <<unknown>>)) -> parsing_error.new()
                                     |> parsing_error.add_message("gwr/parser/parse_limits: unexpected flag value \"" <> int.to_string(unknown) <> "\"")
                                     |> parsing_error.to_error()
            Error(reason) -> Error(reason)
            _ -> parsing_error.new()
                 |> parsing_error.add_message("gwr/parser/parse_limits: unknown error reading flag value")
                 |> parsing_error.to_error()
        }
    )
...
pub fn parse_function_type(from reader: byte_reader.ByteReader) -> Result(#(byte_reader.ByteReader, spec.FunctionType), parsing_error.ParsingError)
{
    use #(reader, function_type) <- result.try(
        case byte_reader.read(from: reader, take: 1)
        {
            Ok(#(reader, <<0x60>>)) ->
            {
                use #(reader, parameters_vec) <- result.try(parse_vector(from: reader, with: parse_value_type))
                use #(reader, results_vec) <- result.try(parse_vector(from: reader, with: parse_value_type))
                Ok(#(reader, spec.FunctionType(parameters: parameters_vec, results: results_vec)))
            }
            Ok(#(_, <<unkown>>)) -> parsing_error.new()
                                    |> parsing_error.add_message("gwr/parser/parse_function_type: unexpected function type id \"" <> int.to_string(unkown) <> "\"")
                                    |> parsing_error.to_error()
            Error(reason) -> Error(reason)
            _ -> parsing_error.new()
                 |> parsing_error.add_message("gwr/parser/parse_function_type: unknown error reading function type id")
                 |> parsing_error.to_error()
        }
    )
...
pub fn parse_global_type(from reader: byte_reader.ByteReader) -> Result(#(byte_reader.ByteReader, spec.GlobalType), parsing_error.ParsingError)
{
    use #(reader, value_type) <- result.try(parse_value_type(from: reader))
    use #(reader, mutability) <- result.try(
        case byte_reader.read(from: reader, take: 1)
        {
            ...
            Ok(#(_, <<unkown>>)) -> parsing_error.new()
                                    |> parsing_error.add_message("gwr/parser/parse_global_type: unexpected mutability flag value \"" <> int.to_string(unkown) <> "\"")
                                    |> parsing_error.to_error()
            Error(reason) -> Error(reason)
            _ -> parsing_error.new()
                 |> parsing_error.add_message("gwr/parser/parse_global_type: unknown error reading mutability flag value")
                 |> parsing_error.to_error()
        }
    )
...
pub fn parse_name(from reader: byte_reader.ByteReader) -> Result(#(byte_reader.ByteReader, spec.Name), parsing_error.ParsingError)
{
