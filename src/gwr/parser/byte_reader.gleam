...
pub opaque type ByteReader {
  ByteReader(data: BitArray, current_position: Int)
}
...
pub fn read(
  ...
  use <- bool.guard(
    when: reader.current_position + count > bit_array.byte_size(reader.data),
    return: parsing_error.new()
      |> parsing_error.add_message("No enough bytes")
      |> parsing_error.to_error(),
  )
  case
    bit_array.slice(at: reader.current_position, from: reader.data, take: count)
  {
    Ok(data_read) ->
      Ok(#(
        ByteReader(
          current_position: reader.current_position + count,
          data: reader.data,
        ),
        data_read,
      ))
...
pub fn do_read_while(
  ...
  case eval(data) {
    True ->
      do_read_while(reader_dirty, eval, bit_array.append(accumulator, data))
...
pub fn get_remaining(
  ...
  case
    bit_array.slice(
      at: reader.current_position,
      from: reader.data,
      take: bit_array.byte_size(reader.data) - reader.current_position,
    )
  {
...
pub fn peek(
  ...
  case
    bit_array.slice(at: reader.current_position, from: reader.data, take: 1)
  {
    Ok(next) -> Ok(next)
    _ ->
      parsing_error.new()
