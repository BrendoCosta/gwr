...
pub fn pop(from stack: Stack) -> #(Stack, option.Option(StackEntry)) {
  #(
    Stack(entries: list.take(from: stack.entries, up_to: length(stack) - 1)),
    peek(stack),
  )
}
      ...
      yielder.fold(
        ...
        with: fn(accumulator, _) {
          ...
          case maybe_popped_entry {
            option.Some(entry) -> #(
              stack_after_pop,
              accumulator.1 |> list.append([entry]),
            )
...
fn do_pop_while(
  accumulator: #(Stack, List(StackEntry)),
  predicate: fn(StackEntry) -> Bool,
  ...
  case peek(from: accumulator.0) {
    option.Some(entry) -> {
      case predicate(entry) {
        True ->
          do_pop_while(
            #(pop(accumulator.0).0, accumulator.1 |> list.append([entry])),
            predicate,
          )
...
pub fn get_current_frame(from stack: Stack) -> Result(spec.Frame, Nil) {
  case
    pop_while(from: stack, with: fn(entry) { !is_activation_frame(entry) }).0
    |> pop
  {
...
pub fn replace_current_frame(
  ...
  let #(stack, upper_entries) =
    pop_while(from: stack, with: fn(entry) { !is_activation_frame(entry) })
  let #(stack, frame) = pop(stack)
  case frame {
    option.Some(ActivationEntry(_)) ->
      Ok(push(
        to: stack,
        push: [ActivationEntry(new_frame)]
          |> list.append(upper_entries |> list.reverse),
      ))
    _ -> Error(Nil)
  }
}
