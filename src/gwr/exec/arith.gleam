  ...
  case j_2 {
    ...
    _ ->
      case j_1 / j_2 {
        ...
        x if x == two_power_n_minus_one ->
          trap.make(trap.Overflow)
          |> trap.to_error()
        _ -> {
...
pub fn irotl(n: Int, i_1: Int, i_2: Int) -> Result(Int, trap.Trap) {
  ...
  let res =
    int.bitwise_or(
      int.bitwise_shift_left(i_1, k),
      int.bitwise_shift_right(i_1, n - k),
    )
  Ok(unsigned(n, res))
}
...
pub fn irotr(n: Int, i_1: Int, i_2: Int) -> Result(Int, trap.Trap) {
  ...
  let res =
    int.bitwise_or(
      int.bitwise_shift_right(i_1, k),
      int.bitwise_shift_left(i_1, n - k),
    )
  Ok(unsigned(n, res))
}
  ...
  case i {
    ...
    _ -> {
      ...
      let #(n, i) = case i <= 0x0000ffff {
        True -> #(
          n + 16,
          int.bitwise_shift_left(i, 16) |> int.bitwise_and(0xffffffff),
        )
        False -> #(n, i)
      }
      let #(n, i) = case i <= 0x00ffffff {
        True -> #(
          n + 8,
          int.bitwise_shift_left(i, 8) |> int.bitwise_and(0xffffffff),
        )
        False -> #(n, i)
      }
      let #(n, i) = case i <= 0x0fffffff {
        True -> #(
          n + 4,
          int.bitwise_shift_left(i, 4) |> int.bitwise_and(0xffffffff),
        )
        False -> #(n, i)
      }
      let #(n, i) = case i <= 0x3fffffff {
        True -> #(
          n + 2,
          int.bitwise_shift_left(i, 2) |> int.bitwise_and(0xffffffff),
        )
        False -> #(n, i)
      }
...
pub fn iclz(n: Int, i: Int) -> Result(Int, trap.Trap) {
  case n {
    ...
    _ ->
      ...
      |> trap.add_message(
        "The iclz operation is currently implemented only for 32 and 64-bit integers",
      )
      |> trap.to_error()
  }
}
...
pub fn ictz(n: Int, i: Int) -> Result(Int, trap.Trap) {
  case n {
    ...
    _ ->
      trap.make(trap.BadArgument)
      |> trap.add_message(
        "The ictz operation is currently implemented only for 32 and 64-bit integers",
      )
      |> trap.to_error()
  }
}
...
fn ipopcnt_32(i: Int) -> Int {
  case i {
    ...
    _ -> {
      let i =
        int.bitwise_and(i, 0x55555555)
        + int.bitwise_and(int.bitwise_shift_right(i, 1), 0x55555555)
      let i =
        int.bitwise_and(i, 0x33333333)
        + int.bitwise_and(int.bitwise_shift_right(i, 2), 0x33333333)
      let i =
        int.bitwise_and(i, 0x0f0f0f0f)
        + int.bitwise_and(int.bitwise_shift_right(i, 4), 0x0f0f0f0f)
      let i =
        int.bitwise_and(i, 0x00ff00ff)
        + int.bitwise_and(int.bitwise_shift_right(i, 8), 0x00ff00ff)
      let i =
        int.bitwise_and(i, 0x0000ffff)
        + int.bitwise_and(int.bitwise_shift_right(i, 16), 0x0000ffff)
      i
    }
  }
}
...
pub fn ipopcnt(n: Int, i: Int) -> Result(Int, trap.Trap) {
  case n {
    ...
    _ ->
      trap.make(trap.BadArgument)
      |> trap.add_message(
        "The ipopcnt operation is currently implemented only for 32 and 64-bit integers",
      )
      |> trap.to_error()
  }
}
