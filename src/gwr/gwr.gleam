...
pub fn load(
  ...
  use #(_, binary) <- result.try(
    parser.parse_binary_module(byte_reader.create(from: data)),
  )
  Ok(binary)
}
...
pub fn call(
  ...
  use function_index <- result.try(
    case
      list.find(in: instance.binary.module.exports, one_that: fn(export) {
    ...
    {
      Ok(spec.Export(name: _, descriptor: spec.FunctionExport(index))) ->
        Ok(index)
      _ ->
  ...
  use #(state, results) <- result.try(exec.invoke(
    instance.state,
    spec.FunctionAddress(function_index),
    arguments,
  ))
