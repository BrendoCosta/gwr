...
pub fn initialize(from module: spec.Module) -> Result(State, String) {
  ...
  let store =
    list.fold(from: store, over: module.memories, with: fn(store, memory) {
      store
  ...
  let module_instance =
    spec.ModuleInstance(
      ...
      function_addresses: store.functions
        ...
        |> dict.from_list,
      table_addresses: [],
      memory_addresses: list.index_map(store.memories, fn(_, index) {
        spec.MemoryAddress(index)
      }),
      global_addresses: [],
      element_addresses: [],
      data_addresses: [],
...
pub fn update_references(
  ...
  let updated_functions =
    dict.map_values(in: store.functions, with: fn(_address, function) {
      case function {
        spec.WebAssemblyFunctionInstance(
          type_: type_,
          module_instance: _,
          code: code,
        ) ->
...
pub fn invoke(
  ...
  let stack =
    stack.push(
      to: state.stack,
      push: arguments |> list.map(fn(x) { stack.ValueEntry(x) }),
    )
  ...
  let results =
    list.map(values, stack.to_value)
    |> result.values
    |> list.reverse
...
pub fn append_web_assembly_function(
  ...
  let function_address =
    spec.FunctionAddress(dict.keys(store.functions) |> list.length)
  ...
  case types_list |> list.take(up_to: function.type_ + 1) |> list.last {
      ...
      Ok(
        spec.Store(
          ...
          functions: dict.insert(
            ...
            insert: spec.WebAssemblyFunctionInstance(
              type_: function_type,
              module_instance: empty_module_instance,
              code: function,
            ),
          ),
        ),
    ...
    Error(_) ->
      Error(
        "gwr/execution/store.append_web_assembly_function: couldn't find the type of the function among types list",
      )
  }
}
...
pub fn append_memory(
  ...
  spec.Store(
    ..store,
    memories: list.append(store.memories, [
      spec.MemoryInstance(type_: memory.type_, data: memory_data),
    ]),
  )
}
...
pub fn ieee_float_to_builtin_float(
  ...
  case ieee_float.is_finite(value) {
    True -> {
      use fin <- result.try(result.replace_error(
        ieee_float.to_finite(value),
        trap.make(trap.Unknown)
          |> trap.add_message(
            "gwr/runtime/ieee_float_to_builtin_float: ieee_float.to_finite error",
          ),
      ))
      Ok(spec.Finite(fin))
    }
    False ->
      case ieee_float.is_nan(value) {
        True -> Ok(spec.NaN)
        False ->
          case ieee_float.compare(value, ieee_float.positive_infinity()) {
            Ok(order.Eq) -> Ok(spec.Infinite(spec.Positive))
            Ok(order.Lt) -> Ok(spec.Infinite(spec.Negative))
            _ ->
              trap.make(trap.Unknown)
              |> trap.add_message(
                "gwr/runtime/ieee_float_to_builtin_float: ieee_float.compare error",
              )
              |> trap.to_error()
          }
      }
...
pub type BinaryOperationType {
  ...
  FloatBinaryOperation(
    fn(ieee_float.IEEEFloat, ieee_float.IEEEFloat) ->
...
pub type ComparisonOperationType {
  ...
  FloatComparisonOperation(
    fn(ieee_float.IEEEFloat, ieee_float.IEEEFloat) -> Result(Int, trap.Trap),
  )
}
...
fn unary_operation(
  ...
  use c <- result.try(case type_, operation_type, values {
    // 3. If {\mathit{unop}}_t(c_1) is defined, then:
    //      3a. Let c be a possible result of computing {\mathit{unop}}_t(c_1).
    spec.Integer32,
      IntegerUnaryOperation(unop),
      option.Some(stack.ValueEntry(spec.Integer32Value(c1)))
    | spec.Integer64,
      IntegerUnaryOperation(unop),
      option.Some(stack.ValueEntry(spec.Integer64Value(c1)))
    -> {
    ...
    spec.Float32,
      FloatUnaryOperation(unop),
      option.Some(stack.ValueEntry(spec.Float32Value(c1)))
    | spec.Float64,
      FloatUnaryOperation(unop),
      option.Some(stack.ValueEntry(spec.Float64Value(c1)))
    -> {
...
fn binary_operation(
  ...
  use c <- result.try(case type_, operation_type, values {
    ...
    spec.Integer32,
      IntegerBinaryOperation(binop),
      [
        stack.ValueEntry(spec.Integer32Value(c2)),
        stack.ValueEntry(spec.Integer32Value(c1)),
      ]
    | spec.Integer64,
      IntegerBinaryOperation(binop),
      [
        stack.ValueEntry(spec.Integer64Value(c2)),
        stack.ValueEntry(spec.Integer64Value(c1)),
      ]
    -> {
    ...
    spec.Float32,
      FloatBinaryOperation(binop),
      [
        stack.ValueEntry(spec.Float32Value(c2)),
        stack.ValueEntry(spec.Float32Value(c1)),
      ]
    | spec.Float64,
      FloatBinaryOperation(binop),
      [
        stack.ValueEntry(spec.Float64Value(c2)),
        stack.ValueEntry(spec.Float64Value(c1)),
      ]
    -> {
...
fn comparison_operation(
  ...
  use c <- result.try(case type_, operation_type, values {
    ...
    spec.Integer32,
      IntegerComparisonOperation(relop),
      [
        stack.ValueEntry(spec.Integer32Value(c2)),
        stack.ValueEntry(spec.Integer32Value(c1)),
      ]
    ...
    spec.Float32,
      FloatComparisonOperation(relop),
      [
        stack.ValueEntry(spec.Float32Value(c2)),
        stack.ValueEntry(spec.Float32Value(c1)),
      ]
...
pub fn evaluate_const(
  ...
  use c <- result.try(case type_, value {
...
pub fn evaluate_iadd(
  ...
  binary_operation(
    stack,
    type_,
    IntegerBinaryOperation(fn(t, a, b) { Ok(arith.iadd(get_bitwidth(t), a, b)) }),
  )
}
...
pub fn evaluate_isub(
  ...
  binary_operation(
    stack,
    type_,
    IntegerBinaryOperation(fn(t, a, b) { Ok(arith.isub(get_bitwidth(t), a, b)) }),
  )
}
...
pub fn evaluate_imul(
  ...
  binary_operation(
    stack,
    type_,
    IntegerBinaryOperation(fn(t, a, b) { Ok(arith.imul(get_bitwidth(t), a, b)) }),
  )
}
...
pub fn evaluate_idiv_u(
  ...
  binary_operation(
    stack,
    type_,
    IntegerBinaryOperation(fn(t, a, b) { arith.idiv_u(get_bitwidth(t), a, b) }),
  )
}
...
pub fn evaluate_idiv_s(
  ...
  binary_operation(
    stack,
    type_,
    IntegerBinaryOperation(fn(t, a, b) { arith.idiv_s(get_bitwidth(t), a, b) }),
  )
}
...
pub fn evaluate_irem_u(
  ...
  binary_operation(
    stack,
    type_,
    IntegerBinaryOperation(fn(t, a, b) { arith.irem_u(get_bitwidth(t), a, b) }),
  )
}
...
pub fn evaluate_irem_s(
  ...
  binary_operation(
    stack,
    type_,
    IntegerBinaryOperation(fn(t, a, b) { arith.irem_s(get_bitwidth(t), a, b) }),
  )
}
...
pub fn evaluate_iand(
  ...
  binary_operation(
    stack,
    type_,
    IntegerBinaryOperation(fn(t, a, b) { Ok(arith.iand(get_bitwidth(t), a, b)) }),
  )
}
...
pub fn evaluate_ior(
  ...
  binary_operation(
    stack,
    type_,
    IntegerBinaryOperation(fn(t, a, b) { Ok(arith.ior(get_bitwidth(t), a, b)) }),
  )
}
...
pub fn evaluate_ixor(
  ...
  binary_operation(
    stack,
    type_,
    IntegerBinaryOperation(fn(t, a, b) { Ok(arith.ixor(get_bitwidth(t), a, b)) }),
  )
}
...
pub fn evaluate_ishl(
  ...
  binary_operation(
    stack,
    type_,
    IntegerBinaryOperation(fn(t, a, b) { arith.ishl(get_bitwidth(t), a, b) }),
  )
}
...
pub fn evaluate_ishr_u(
  ...
  binary_operation(
    stack,
    type_,
    IntegerBinaryOperation(fn(t, a, b) { arith.ishr_u(get_bitwidth(t), a, b) }),
  )
}
...
pub fn evaluate_ishr_s(
  ...
  binary_operation(
    stack,
    type_,
    IntegerBinaryOperation(fn(t, a, b) { arith.ishr_s(get_bitwidth(t), a, b) }),
  )
}
...
pub fn evaluate_irotl(
  ...
  binary_operation(
    stack,
    type_,
    IntegerBinaryOperation(fn(t, a, b) { arith.irotl(get_bitwidth(t), a, b) }),
  )
}
...
pub fn evaluate_irotr(
  ...
  binary_operation(
    stack,
    type_,
    IntegerBinaryOperation(fn(t, a, b) { arith.irotr(get_bitwidth(t), a, b) }),
  )
}
...
pub fn evaluate_iclz(
  ...
  unary_operation(
    stack,
    type_,
    IntegerUnaryOperation(fn(t, a) { arith.iclz(get_bitwidth(t), a) }),
  )
}
...
pub fn evaluate_ictz(
  ...
  unary_operation(
    stack,
    type_,
    IntegerUnaryOperation(fn(t, a) { arith.ictz(get_bitwidth(t), a) }),
  )
}
...
pub fn evaluate_ipopcnt(
  ...
  unary_operation(
    stack,
    type_,
    IntegerUnaryOperation(fn(t, a) { arith.ipopcnt(get_bitwidth(t), a) }),
  )
}
...
pub fn evaluate_ieqz(
  ...
  test_operation(
    stack,
    type_,
    IntegerTestOperation(fn(a) { Ok(arith.ieqz(a)) }),
  )
}
...
pub fn evaluate_ieq(
  ...
  comparison_operation(
    stack,
    type_,
    IntegerComparisonOperation(fn(_, a, b) { Ok(arith.ieq(a, b)) }),
  )
}
...
pub fn evaluate_ine(
  ...
  comparison_operation(
    stack,
    type_,
    IntegerComparisonOperation(fn(_, a, b) { Ok(arith.ine(a, b)) }),
  )
}
...
pub fn evaluate_ilt_u(
  ...
  comparison_operation(
    stack,
    type_,
    IntegerComparisonOperation(fn(t, a, b) {
      Ok(arith.ilt_u(get_bitwidth(t), a, b))
    }),
  )
}
...
pub fn evaluate_ilt_s(
  ...
  comparison_operation(
    ...
    type_,
    IntegerComparisonOperation(fn(t, a, b) {
      Ok(arith.ilt_s(get_bitwidth(t), a, b))
    }),
  )
}
...
pub fn evaluate_igt_u(
  ...
  comparison_operation(
    ...
    type_,
    IntegerComparisonOperation(fn(t, a, b) {
      Ok(arith.igt_u(get_bitwidth(t), a, b))
    }),
  )
}
...
pub fn evaluate_igt_s(
  ...
  comparison_operation(
    ...
    type_,
    IntegerComparisonOperation(fn(t, a, b) {
      Ok(arith.igt_s(get_bitwidth(t), a, b))
    }),
  )
}
...
pub fn evaluate_ile_u(
  ...
  comparison_operation(
    ...
    type_,
    IntegerComparisonOperation(fn(t, a, b) {
      Ok(arith.ile_u(get_bitwidth(t), a, b))
    }),
  )
}
...
pub fn evaluate_ile_s(
  ...
  comparison_operation(
    ...
    type_,
    IntegerComparisonOperation(fn(t, a, b) {
      Ok(arith.ile_s(get_bitwidth(t), a, b))
    }),
  )
}
...
pub fn evaluate_ige_u(
  ...
  comparison_operation(
    ...
    type_,
    IntegerComparisonOperation(fn(t, a, b) {
      Ok(arith.ige_u(get_bitwidth(t), a, b))
    }),
  )
}
...
pub fn evaluate_ige_s(
  ...
  comparison_operation(
    ...
    type_,
    IntegerComparisonOperation(fn(t, a, b) {
      Ok(arith.ige_s(get_bitwidth(t), a, b))
    }),
  )
}
...
pub fn evaluate_iextend8_s(
  ...
  unary_operation(
    stack,
    type_,
    IntegerUnaryOperation(fn(t, a) { arith.iextend8_s(get_bitwidth(t), a) }),
  )
}
...
pub fn evaluate_iextend16_s(
  ...
  unary_operation(
    stack,
    type_,
    IntegerUnaryOperation(fn(t, a) { arith.iextend16_s(get_bitwidth(t), a) }),
  )
}
...
pub fn evaluate_iextend32_s(
  ...
  unary_operation(
    stack,
    type_,
    IntegerUnaryOperation(fn(t, a) { arith.iextend32_s(get_bitwidth(t), a) }),
  )
}

pub fn evaluate_local_get(
  stack: stack.Stack,
  index: spec.LocalIndex,
) -> Result(stack.Stack, trap.Trap) {
  // 1. Let F be the current frame.
  use frame <- result.try(result.replace_error(
    stack.get_current_frame(from: stack),
    trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.evaluate_local_get: couldn't get the current frame",
      ),
  ))
  // 2. Assert: due to validation, F.{\mathsf{locals}}[x] exists.
  // 3. Let {\mathit{val}} be the value F.{\mathsf{locals}}[x].
  use local <- result.try(result.replace_error(
    dict.get(frame.framestate.locals, index),
    trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.evaluate_local_get: couldn't get the local with index "
        <> int.to_string(index),
      ),
  ))
...
pub fn evaluate_local_set(
  ...
  use frame <- result.try(result.replace_error(
    ...
    trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.evaluate_local_set: couldn't get the current frame",
      ),
  ))
  ...
  use _ <- result.try(result.replace_error(
    ...
    trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.evaluate_local_set: couldn't get the local with index "
        <> int.to_string(index),
      ),
  ))
  ...
  use #(stack, value) <- result.try(
    stack.pop_as(from: stack, with: stack.to_value)
    |> result.replace_error(
      trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.evaluate_local_set: couldn't pop the value from stack",
      ),
    ),
  )
  ...
  use stack <- result.try(
    ...
    |> result.replace_error(
      trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.evaluate_local_set: couldn't update the current frame",
      ),
    ),
  )
  Ok(stack)
...
pub fn evaluate_local_tee(
  ...
  case stack.pop(from: stack) {
    #(stack, option.Some(stack.ValueEntry(val))) -> {
      ...
      let stack =
        stack.push(to: stack, push: [
          stack.ValueEntry(val),
          stack.ValueEntry(val),
        ])
    ...
    _ ->
      trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.evaluate_local_tee: expected a value on the top of the stack",
      )
      |> trap.to_error()
  }
}
...
fn get_default_value_for_type(type_: spec.ValueType) -> spec.Value {
  case type_ {
    spec.Number(spec.Integer32) ->
      spec.Integer32Value(spec.number_value_default_value)
    spec.Number(spec.Integer64) ->
      spec.Integer64Value(spec.number_value_default_value)
    spec.Number(spec.Float32) ->
      spec.Float32Value(
        spec.Finite(int.to_float(spec.number_value_default_value)),
      )
    ...
    spec.Vector(spec.Vector128) ->
      spec.VectorValue(spec.vector_value_default_value)
    spec.Reference(spec.FunctionReference) ->
      spec.ReferenceValue(spec.NullReference)
...
pub fn evaluate_return(
  ...
  use frame <- result.try(
    ...
    |> result.replace_error(
      trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.evaluate_return: couldn't get the current frame",
      ),
    ),
  )
  ...
  let count_of_values_on_top =
    stack.count_on_top(from: stack, with: stack.is_value)
  use <- bool.guard(
    when: count_of_values_on_top < n,
    return: trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.evaluate_return: expected the top of the stack to contains at least "
        <> int.to_string(n)
        <> " values but got "
        <> int.to_string(count_of_values_on_top),
      )
      |> trap.to_error(),
  )
  ...
  use <- bool.guard(
    when: stack.get_entries(stack)
    |> list.filter(stack.is_activation_frame)
    |> list.length
      <= 0,
    return: trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.evaluate_return: expected the stack to contains at least one frame",
      )
      |> trap.to_error(),
  )
  ...
  let #(stack, _) =
    stack.pop_while(from: stack, with: fn(entry) {
      !stack.is_activation_frame(entry)
    })
  // 7. Assert: the top of the stack is the frame F.
  use <- bool.guard(
    when: stack.peek(stack) != option.Some(stack.ActivationEntry(frame)),
    return: trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.evaluate_return: expected the top of the stack to be the current frame",
      )
      |> trap.to_error(),
  )
...
pub fn evaluate_call(
  ...
  use frame <- result.try(
    ...
    |> result.replace_error(
      trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.evaluate_call: couldn't get the current frame",
      ),
    ),
  )
  ...
  use address <- result.try(
    ...
    |> result.replace_error(
      trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.evaluate_call: couldn't find the address of the function with index "
        <> int.to_string(index),
      ),
    ),
  )
...
pub fn evaluate_invoke(
  ...
  use function_instance <- result.try(
    ...
    |> result.replace_error(
      trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.invoke: couldn't find the function instance with address "
        <> address_to_string(address),
      ),
    ),
  )
  case function_instance {
    spec.HostFunctionInstance(_, _) ->
      trap.make(trap.BadArgument)
    ...
    spec.WebAssemblyFunctionInstance(
      ...
      use <- bool.guard(
        ...
        return: trap.make(trap.InvalidState)
          |> trap.add_message(
            "gwr/execution/evaluator.invoke: expected the top of the stack to contains "
            <> int.to_string(n)
            <> " values but got "
            <> int.to_string(count_of_values_on_top),
          )
      ...
      let framestate =
        spec.FrameState(
          locals: values
            ...
            // function's arguments will be joined with function's locals
      ...
      evaluate_with_frame(
        stack,
        store,
        spec.Frame(arity: m, framestate: framestate),
...
pub fn evaluate_with_frame(
  ...
  use #(stack, store, jump) <- result.try(
    enter_with_label(stack, store, label, instructions, []),
  )
    ...
    _ -> {
      ...
      use frame <- result.try(
        ...
        |> result.replace_error(
          trap.make(trap.InvalidState)
          |> trap.add_message(
            "gwr/execution/evaluator.evaluate_with_frame: couldn't get the current frame",
          ),
        ),
      )
      ...
      use <- bool.guard(
        ...
        return: trap.make(trap.InvalidState)
          |> trap.add_message(
            "gwr/execution/evaluator.evaluate_with_frame: expected the top of the stack to contains "
            <> int.to_string(n)
            <> " values but got "
            <> int.to_string(count_of_values_on_top),
          )
      ...
      use <- bool.guard(
        ...
        return: trap.make(trap.InvalidState)
          |> trap.add_message(
            "gwr/execution/evaluator.evaluate_with_frame: expected the current frame to be on the top of the stack",
          )
          |> trap.to_error(),
      )
...
pub fn evaluate_br(
  stack: stack.Stack,
  store: spec.Store,
  index: spec.LabelIndex,
) -> Result(#(stack.Stack, spec.Store, option.Option(Jump)), trap.Trap) {
  ...
  use <- bool.guard(
    when: count_of_labels_in_stack < index + 1,
    return: trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.evaluate_br: expected the stack to contains at least "
        <> int.to_string(index + 1)
        <> " labels but got "
        <> int.to_string(count_of_labels_in_stack),
      )
      |> trap.to_error(),
  )
  ...
  use label_entry <- result.try(
    ...
    |> result.replace_error(
      trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.evaluate_br: couldn't find the label with index "
        <> int.to_string(index),
      ),
    ),
  )
  ...
  use <- bool.guard(
    ...
    return: trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.evaluate_br: expected the top of the stack to contains at least "
        <> int.to_string(n)
        <> " values but got "
        <> int.to_string(count_of_values_on_top),
      )
      ...
      with: fn(accumulator, _) {
        ...
        case stack.pop(from: stack) {
          ...
          #(_, anything_else) ->
            trap.make(trap.InvalidState)
            |> trap.add_message(
              "gwr/execution/evaluator.evaluate_br: expected the top of the stack to contain a label but got "
              <> string.inspect(anything_else),
            )
            |> trap.to_error()
        }
      },
...
pub fn evaluate_br_if(
  ...
  case stack.pop(from: stack) {
    ...
    _ ->
      trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.evaluate_br_if: expected the top of the stack to contain an i32 value",
      )
      |> trap.to_error()
  }
}
...
pub fn evaluate_loop(
  ...
  use frame <- result.try(
    ...
    |> result.replace_error(
      trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.evaluate_loop: couldn't get the current frame",
      ),
    ),
  )
  ...
  use function_type <- result.try(expand_block_type(
    frame.framestate,
    block_type,
  ))
  ...
  let label =
    spec.Label(arity: m, continuation: [
      spec.Loop(block_type: block_type, instructions: instructions),
    ])
  // 5. Assert: due to validation, there are at least m values on the top of the stack.
  let count_of_values_on_top =
    stack.count_on_top(from: stack, with: stack.is_value)
  use <- bool.guard(
    when: count_of_values_on_top < m,
    return: trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.evaluate_loop: expected the top of the stack to contains at least "
        <> int.to_string(m)
        <> " values but got "
        <> int.to_string(count_of_values_on_top),
      )
...
pub fn evaluate_block(
  ...
  use frame <- result.try(
    ...
    |> result.replace_error(
      trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.evaluate_block: couldn't get the current frame",
      ),
    ),
  )
  // 2. Assert: due to validation, {\mathrm{expand}}_F({\mathit{blocktype}}) is defined.
  ...
  use <- bool.guard(
    ...
    return: trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.evaluate_block: expected the top of the stack to contains at least "
        <> int.to_string(m)
        <> " values but got "
        <> int.to_string(count_of_values_on_top),
      )
...
pub fn enter_with_label(
  ...
  evaluate_expression(
    stack.push(
      to: stack,
      push: [stack.LabelEntry(label)] |> list.append(parameters),
    ),
    store,
    instructions,
  )
...
pub fn exit_with_label(
  ...
  use stack <- result.try(case stack.pop(stack) {
    #(stack, option.Some(stack.LabelEntry(some_label))) if some_label == label ->
      Ok(stack)
    #(_, anything_else) ->
      trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.exit_with_label: expected the label "
        <> string.inspect(label)
        <> " pushed to the stack before execution but got "
        <> string.inspect(anything_else),
      )
      |> trap.to_error()
  })
    ...
    _ -> {
      use instruction <- result.try(
        ...
        |> result.replace_error(
          trap.make(trap.Unknown)
          |> trap.add_message(
            "gwr/execution/evaluator.evaluate_expression: couldn't get the current instruction",
          ),
        ),
      )
      use #(stack, store, jump) <- result.try(case instruction {
        ...
        spec.Block(block_type:, instructions:) ->
          evaluate_block(stack, store, block_type, instructions)
        spec.If(block_type:, instructions:, else_: else_) ->
          evaluate_if_else(stack, store, block_type, instructions, else_)
        spec.Loop(block_type:, instructions:) ->
        ...
        spec.Return ->
          evaluate_return(stack) |> result.map(fn(x) { #(x.0, store, x.1) })
        spec.Call(index:) ->
          evaluate_call(stack, store, index)
          |> result.map(fn(x) { #(x.0, x.1, option.None) })
        ...
        _ ->
          result.map(
            case instruction {
              ...
              spec.I32Const(value) ->
                evaluate_const(stack, spec.Integer32, IntegerConstValue(value))
              spec.I64Const(value) ->
                evaluate_const(stack, spec.Integer64, IntegerConstValue(value))
              ...
              unknown ->
                trap.make(trap.BadArgument)
                |> trap.add_message(
                  "gwr/execution/evaluator.evaluate_expression: attempt to execute an unknown or unimplemented instruction \""
      ...
      case jump {
        ...
        option.Some(Branch(target: instructions)) ->
          evaluate_expression(stack, store, instructions)
        option.None ->
          evaluate_expression(stack, store, instructions |> list.drop(1))
      }
    }
  }
}
  ...
  case stack.pop_as(from: stack, with: stack.to_value) {
    Ok(#(stack, spec.Integer32Value(c))) -> {
      case c != 0 {
        ...
        False ->
          case else_ {
            option.Some(spec.Else(else_instructions)) ->
              evaluate_block(stack, store, block_type, else_instructions)
            option.None -> Ok(#(stack, store, option.None))
            anything_else ->
              trap.make(trap.BadArgument)
              |> trap.add_message(
                "gwr/execution/evaluator.evaluate_if_else: illegal instruction in the Else's field "
                <> string.inspect(anything_else),
              )
              |> trap.to_error()
          }
    ...
    anything_else ->
      trap.make(trap.InvalidState)
      |> trap.add_message(
        "gwr/execution/evaluator.evaluate_if_else: expected the If's continuation flag but got "
        <> string.inspect(anything_else),
      )
      |> trap.to_error()
  }
...
pub fn expand_block_type(
  ...
  case block_type {
    spec.TypeIndexBlock(index) ->
      framestate.module_instance.types
      |> list.take(up_to: index + 1)
      |> list.last
      |> result.replace_error(
        trap.make(trap.InvalidState)
        |> trap.add_message(
          "gwr/execution/evaluator.expand_block_type: couldn't find the function type with index \""
          <> int.to_string(index)
          <> "\"",
        ),
      )
    spec.ValueTypeBlock(type_: option.Some(valtype)) ->
      Ok(spec.FunctionType(parameters: [], results: [valtype]))
    spec.ValueTypeBlock(type_: option.None) ->
      Ok(spec.FunctionType(parameters: [], results: []))
  }
}
