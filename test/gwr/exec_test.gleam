...
{
    exec.evaluate_const(stack.create(), spec.Integer32, exec.IntegerConstValue(65536))
    |> should.be_ok
    |> should.equal(stack.push(stack.create(), [stack.ValueEntry(spec.Integer32Value(65536))]))
}

pub fn evaluate_const___i64___test()
{
    exec.evaluate_const(stack.create(), spec.Integer64, exec.IntegerConstValue(65536))
    |> should.be_ok
    |> should.equal(stack.push(stack.create(), [stack.ValueEntry(spec.Integer64Value(65536))]))
}

pub fn evaluate_const___f32___test()
{
    exec.evaluate_const(stack.create(), spec.Float32, exec.FloatConstValue(ieee_float.finite(65536.0)))
    |> should.be_ok
    |> should.equal(stack.push(stack.create(), [stack.ValueEntry(spec.Float32Value(spec.Finite(65536.0)))]))
}

pub fn evaluate_const___f64___test()
{
    exec.evaluate_const(stack.create(), spec.Float64, exec.FloatConstValue(ieee_float.finite(65536.0)))
    |> should.be_ok
    |> should.equal(stack.push(stack.create(), [stack.ValueEntry(spec.Float64Value(spec.Finite(65536.0)))]))
}

pub fn evaluate_const___bad_argument_1___test()
{
    exec.evaluate_const(stack.create(), spec.Integer32, exec.FloatConstValue(ieee_float.finite(65536.0)))
    |> should.be_error
}

pub fn evaluate_const___bad_argument_2___test()
{
    exec.evaluate_const(stack.create(), spec.Float64, exec.IntegerConstValue(65536))
    |> should.be_error
}
...
pub fn evaluate_local_get_test()
{
    ...
    |> should.be_ok
    |> should.equal(stack.push(test_stack, [stack.ValueEntry(spec.Integer32Value(16))]))
}
...
pub fn evaluate_return___return_flag___test()
{
    ...
    |> stack.push([
        ...
        stack.LabelEntry(
            spec.Label(arity: 0, continuation: [])
        )
    ])
...
pub fn evaluate_return_test()
{
    ...
    |> stack.push([
        ...
        stack.LabelEntry(
            spec.Label(arity: 2, continuation: [])
        ),
        ...
        stack.LabelEntry(
            spec.Label(arity: 3, continuation: [])
        ),
        stack.ValueEntry(spec.Integer32Value(128)),
        stack.ValueEntry(spec.Integer32Value(256)),
        stack.ValueEntry(spec.Integer32Value(512))
    ])
    |> exec.evaluate_return() // Return from Function Call #3
    |> should.be_ok
    |> pair.first
    |> exec.evaluate_return() // Return from Function Call #2
    |> should.be_ok
    |> should.equal(
        #(
            stack.create()
