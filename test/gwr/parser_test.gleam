...
pub fn parse_binary_module___basic_add___test()
{
    ...
    |> should.equal(
        spec.Binary(
            ...
            module: spec.Module
            (
                types: [spec.FunctionType([spec.Number(spec.Integer32), spec.Number(spec.Integer32)], [spec.Number(spec.Integer32)])],
                functions: [spec.Function(0, [], [spec.LocalGet(0), spec.LocalGet(1), spec.I32Add, spec.End])],
                tables: [],
                memories: [],
                globals: [],
                elements: [],
...
{
    parser.parse_binary_module(byte_reader.create(from: <<0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00>>))
    |> should.be_error
    |> parsing_error.get_message
    |> should.be_some
    |> should.equal("gwr/parser/parser.parse_binary_module: couldn't find module's magic number")
}

pub fn parse_binary_module___could_not_find_module_magic_number_2___test()
{
    parser.parse_binary_module(byte_reader.create(from: <<0x00, 0x63, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00>>))
    |> should.be_error
    |> parsing_error.get_message
    |> should.be_some
    |> should.equal("gwr/parser/parser.parse_binary_module: couldn't find module's magic number")
...
{
    parser.parse_binary_module(byte_reader.create(from: <<0x00, 0x61, 0x73, 0x6d>>))
    |> should.be_error
    |> parsing_error.get_message
    |> should.be_some
    |> should.equal("gwr/parser/parser.parse_binary_module: couldn't find module version")
}

pub fn parse_binary_module___could_not_find_module_version_2___test()
{
    parser.parse_binary_module(byte_reader.create(from: <<0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00>>))
    |> should.be_error
    |> parsing_error.get_message
    |> should.be_some
    |> should.equal("gwr/parser/parser.parse_binary_module: couldn't find module version")
...
pub fn parse_section___unexpected_end___test()
{
    ...
    |> parsing_error.get_message
    |> should.be_some
    |> should.equal("gwr/parser/parser.parse_section: unexpected end of the section's content segment. Expected 9 bytes but got 8 bytes")
}
        ...
        spec.Section(
            ...
            content: Some(spec.TypeSection(
                function_types: [
                    ...
                    spec.FunctionType(parameters: [spec.Reference(spec.FunctionReference), spec.Reference(spec.ExternReference), spec.Number(spec.Integer32)], results: [spec.Reference(spec.ExternReference), spec.Number(spec.Integer64)]),
                ...
                entries: [
                    spec.Code(
                        ...
                        function_code: spec.FunctionCode(
                            locals: [spec.LocalsDeclaration(count: 2, type_: spec.Number(spec.Integer32))],
                            body: [
...
pub fn parse_locals_declaration___3_integer32___test()
{
    ...
    |> should.be_ok
    |> pair.second
    |> should.equal(spec.LocalsDeclaration(count: 3, type_: spec.Number(spec.Integer32)))
}
...
pub fn parse_locals_declaration___2_integer64___test()
{
    ...
    |> should.be_ok
    |> pair.second
    |> should.equal(spec.LocalsDeclaration(count: 2, type_: spec.Number(spec.Integer64)))
}
...
pub fn parse_locals_declaration___128_float32___test()
{
    ...
    |> should.be_ok
    |> pair.second
    |> should.equal(spec.LocalsDeclaration(count: 128, type_: spec.Number(spec.Float32)))
}
...
pub fn parse_locals_declaration___123456_float64___test()
{
    ...
    |> should.be_ok
    |> pair.second
    |> should.equal(spec.LocalsDeclaration(count: 123456, type_: spec.Number(spec.Float64)))
}
...
pub fn parse_locals_declaration___255_vector128___test()
{
    ...
    |> should.be_ok
    |> pair.second
    |> should.equal(spec.LocalsDeclaration(count: 255, type_: spec.Vector(spec.Vector128)))
}
...
pub fn parse_locals_declaration___2_function_reference___test()
{
    ...
    |> should.be_ok
    |> pair.second
    |> should.equal(spec.LocalsDeclaration(count: 2, type_: spec.Reference(spec.FunctionReference)))
}
...
pub fn parse_locals_declaration___1_extern_reference___test()
{
    ...
    |> should.be_ok
    |> pair.second
    |> should.equal(spec.LocalsDeclaration(count: 1, type_: spec.Reference(spec.ExternReference)))
}
...
pub fn parse_function_code_test()
{
    let reader = byte_reader.create(from: <<
        0x07,                           // A vector with 7 LocalsDeclaration
            0x03,               0x7f,   // LocalsDeclaration(count: 3, type_: spec.Number(spec.Integer32))
            0x02,               0x7e,   // LocalsDeclaration(count: 2, type_: spec.Number(spec.Integer64))
            0x80, 0x01,         0x7d,   // LocalsDeclaration(count: 128, type_: spec.Number(spec.Float32))
            0xc0, 0xc4, 0x07,   0x7c,   // LocalsDeclaration(count: 123456, type_: spec.Number(spec.Float64))
            0xff, 0x01,         0x7b,   // LocalsDeclaration(count: 255, type_: spec.Vector(spec.Vector128))
            0x02,               0x70,   // LocalsDeclaration(count: 2, type_: spec.Reference(spec.FunctionReference))
            0x01,               0x6f,   // LocalsDeclaration(count: 1, type_: spec.Reference(spec.ExternReference))
    ...
    |> should.equal(
        spec.FunctionCode(
            locals: [
                ...
                spec.LocalsDeclaration(count: 123456, type_: spec.Number(spec.Float64)),
                spec.LocalsDeclaration(count: 255, type_: spec.Vector(spec.Vector128)),
                spec.LocalsDeclaration(count: 2, type_: spec.Reference(spec.FunctionReference)),
                spec.LocalsDeclaration(count: 1, type_: spec.Reference(spec.ExternReference)),
            ],
            body: [
                spec.NoOp,
                spec.NoOp,
...
pub fn parse_block_type___value_type_block___test()
{
    ...
    |> should.be_ok
    |> pair.second
    |> should.equal(spec.ValueTypeBlock(type_: option.Some(spec.Number(spec.Integer32))))
}
...
pub fn parse_block_type___type_index_block___test()
{
    ...
    |> should.be_ok
    |> pair.second
    |> should.equal(spec.TypeIndexBlock(index: 65536))
}
...
pub fn parse_instruction___block___test()
{
    ...
    |> should.equal(
        spec.Block(
            block_type: spec.TypeIndexBlock(index: 1),
            instructions: [
                spec.I32Const(value: 1048576),
...
pub fn parse_instruction___loop___test()
{
    ...
    |> should.equal(
        spec.Loop(
            block_type: spec.ValueTypeBlock(type_: option.Some(spec.Number(spec.Integer32))),
            instructions: [
                spec.I32Const(value: 8),
                spec.I32Const(value: 65536),
                spec.End
            ]
        )
    )
...
pub fn parse_instruction___i32_const___test()
{
    ...
    |> should.be_ok
    |> pair.second
    |> should.equal(spec.I32Const(value: 1048576))
}
...
pub fn parse_instruction___local_tee___test()
{
    ...
    |> should.be_ok
    |> pair.second
    |> should.equal(spec.LocalGet(index: 65536))
}
...
pub fn parse_instruction___br_if___test()
{
    ...
    |> should.be_ok
    |> pair.second
    |> should.equal(spec.BrIf(index: 65536))
}
...
{
    let reader = byte_reader.create(from: <<0x0b, "my_function":utf8, 0x00, 0x00>>)
    parser.parse_export(reader)
    |> should.be_ok
    |> pair.second
    |> should.equal(spec.Export(name: "my_function", descriptor: spec.FunctionExport(index: 0)))
}

pub fn parse_export___table_export___test()
{
    let reader = byte_reader.create(from: <<0x08, "my_table":utf8, 0x01, 0xc0, 0xc4, 0x07>>)
    parser.parse_export(reader)
    |> should.be_ok
    |> pair.second
    |> should.equal(spec.Export(name: "my_table", descriptor: spec.TableExport(index: 123456)))
}

pub fn parse_export___memory_export___test()
{
    let reader = byte_reader.create(from: <<0x09, "my_memory":utf8, 0x02, 0xff, 0x01>>)
    parser.parse_export(reader)
    |> should.be_ok
    |> pair.second
    |> should.equal(spec.Export(name: "my_memory", descriptor: spec.MemoryExport(index: 255)))
}

pub fn parse_export___global_export___test()
{
    let reader = byte_reader.create(from: <<0x09, "my_global":utf8, 0x03, 0xff, 0xff, 0xff, 0xff, 0x0f>>)
    parser.parse_export(reader)
    |> should.be_ok
    |> pair.second
    |> should.equal(spec.Export(name: "my_global", descriptor: spec.GlobalExport(index: 4294967295)))
}
...
pub fn parse_global_type___constant___test()
{
    ...
    |> should.be_ok
    |> pair.second
    |> should.equal(spec.GlobalType(value_type: spec.Number(spec.Integer32), mutability: spec.Constant))
}
...
pub fn parse_global_type___variable___test()
{
    ...
    |> should.be_ok
    |> pair.second
    |> should.equal(spec.GlobalType(value_type: spec.Number(spec.Integer64), mutability: spec.Variable))
}
