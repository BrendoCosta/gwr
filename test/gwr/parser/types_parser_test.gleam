...
{
    let reader = binary_reader.create(from: <<0x60, 0x00, 0x00>>)
    types_parser.parse_function_type(reader)
    |> should.be_ok
    |> should.equal(
        #(
...
{
    let reader = binary_reader.create(from: <<0x7f, 0x00>>)
...
{
    let reader = binary_reader.create(from: <<0x7e, 0x01>>)
    types_parser.parse_global_type(reader)
    |> should.be_ok
    |> should.equal(
        #(
...
{
    let reader = binary_reader.create(from: <<0x00, 0x03>>)
...
{
    let reader = binary_reader.create(from: <<0x01, 0x20, 0x80, 0x02>>)
    types_parser.parse_limits(reader)
    |> should.be_ok
    |> should.equal(
        #(
...
{
    let reader = binary_reader.create(from: <<0x7f>>)
    types_parser.parse_value_type(reader)
    |> should.be_ok
    |> should.equal(
        #(
            binary_reader.BinaryReader(..reader, current_position: 1),
            types.Number(types.Integer32)
        )
    )
}
...
{
    let reader = binary_reader.create(from: <<0x7e>>)
    types_parser.parse_value_type(reader)
    |> should.be_ok
    |> should.equal(
        #(
...
{
    let reader = binary_reader.create(from: <<0x7d>>)
    types_parser.parse_value_type(reader)
    |> should.be_ok
    |> should.equal(
        #(
...
{
    let reader = binary_reader.create(from: <<0x7c>>)
    types_parser.parse_value_type(reader)
    |> should.be_ok
    |> should.equal(
        #(
...
{
    let reader = binary_reader.create(from: <<0x7b>>)
    types_parser.parse_value_type(reader)
    |> should.be_ok
    |> should.equal(
        #(
...
{
    let reader = binary_reader.create(from: <<0x70>>)
    types_parser.parse_value_type(reader)
    |> should.be_ok
    |> should.equal(
        #(
...
{
    let reader = binary_reader.create(from: <<0x6f>>)
    types_parser.parse_value_type(reader)
    |> should.be_ok
    |> should.equal(
        #(
